...
Komennolla ```sudo mkdir -p /srv/salt/hello``` luodaan kansio "hello" meidän SLStiedostolle. Seuraavaksi luodaan SLS-tiedosto komennolla ```sudoedit /srv/salt/hello/init.sls```. Haluamme että minioneilla on meidän testi.txt -niminen tiedosto heillä /home -kansioissa.
